...
fig <- summary(margins(res, at = list(opinion_strength = 1:7)))  %>% 
  ...
  labs(x = 'Average marginal treatment effect',
       ...
       subtitle = 'Outcome measure: Fairness perception of decision.') +
  theme_bw() +
...
ggsave(
  here("output", "swevig", "figs", "pngs", "exp1-fairness-importance-int.png"),
  plot = fig,
  width = 7, height = 2.75
)

ggsave(
  here("output", "swevig", "figs", "pdfs", "exp1-fairness-importance-int.pdf"),
  plot = fig,
  width = 7, height = 2.75
)
  
  
table <- summary(margins(res)) %>%
...
kable(table, booktabs = TRUE, caption = "Moderating effects of issue importance on experimental treatment, Study 1 -- Swedish vignette", digits = 2, col.names = linebreak(c("Factor", "AME", "SE", "z-statistic", "p value", 'Lower', 'Upper'))) %>% 
  kable_styling(latex_options = "HOLD_position")   %>% 
  cat(., file = here("output", "swevig", "tbls", "html", "tbl-fairness-importance-int-swevig.html"))
...
kable(table, booktabs = TRUE, caption = "Moderating effects of issue importance, Study 1 -- Swedish vignette", digits = 2, col.names = linebreak(c("Treatment value", "Estimate", "Std. Error", "t-statistic", "p value"))) %>% 
  kable_styling(latex_options = "HOLD_position") %>% 
   cat(., file = here("output", "swevig", "tbls", "html", "tbl-fairness-importance-int2-swevig.html"))
