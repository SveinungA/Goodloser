...
d %>% group_by(r13pad2) %>%
   summarize(N_obs = n()) %>% 
  mutate(Percent_obs = round(N_obs/(sum(N_obs))*100)) %>%
  kable(booktabs = TRUE, linesep = "", escape = F, caption = "How important is the issue of begging ban to you?", col.names = linebreak(c("Value", "N", "Percent"))) %>% 
  kable_styling(latex_options = "HOLD_position")%>% 
...
d %>% group_by(r13pad3) %>%
   summarize(N_obs = n()) %>% 
  mutate(Percent_obs = round(N_obs/(sum(N_obs))*100)) %>%
  kable(booktabs = TRUE, linesep = "", escape = F, caption = "What is your opinion on an increase in the tolls for diesel cars in your municipality?", col.names = linebreak(c("Value", "N", "Percent"))) %>% 
  kable_styling(latex_options = "HOLD_position")%>% 
...
d %>% group_by(r13pad4) %>%
   summarize(N_obs = n()) %>% 
  mutate(Percent_obs = round(N_obs/(sum(N_obs))*100)) %>%
  kable(booktabs = TRUE, linesep = "", escape = F, caption = "How important is the issue of increased tolls for diesel cars to you?", col.names = linebreak(c("Value", "N", "Percent"))) %>% 
  kable_styling(latex_options = "HOLD_position") %>% 
  cat(., file = here("output", "nocon", "tbls", "html",  "tbl-nocon-toll-preference.html"))
...
table <- tibble(Treatments = c("Issue", "Issue", "Outcome", "Outcome", "Winning margin", "Winning margin", "Winning margin", "Winner's gloating", "Winner's gloating", rep("Messenger and prime", 7)), 
                Value = c("Ban on begging", 
                          ...
                          "Not shown", 
                          "Politician, no prime", 
                          "Politician,  specific prime",
                          "Politician, general prime", 
                          "Newspaper, no prime",
                          "Newspaper, specific prime", 
                          "Newspaper, general prime"), 
...
d %>% group_by(r13pad7a) %>%
   summarize(N_obs = n()) %>% 
  mutate(Percent_obs = round(N_obs/(sum(N_obs))*100)) %>%
  kable(booktabs = TRUE, linesep = "", escape = F, caption = "How reasonable do you think the decision was?", col.names = linebreak(c("Value", "N", "Percent"))) %>% 
  kable_styling(latex_options = "HOLD_position") %>% 
...
d %>% group_by(r13pad8a) %>%
   summarize(N_obs = n()) %>% 
  mutate(Percent_obs = round(N_obs/(sum(N_obs))*100)) %>%
  kable(booktabs = TRUE, linesep = "", escape = F, caption = "When you think about the actual outcome of the decision, how willing are you to accept the decision?", col.names = linebreak(c("Value", "N", "Percent"))) %>%   kable_styling(latex_options = "HOLD_position") %>% 
