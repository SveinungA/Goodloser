...
## We want the value labels in particular order for the tables and figures.
    ...
    main_02 <- main_01 %>% pivot_longer(cols= contains('_imp'), names_to = 'variable', values_to = 'importance', values_drop_na = TRUE)
``` 
...
```{r 306_post_fair_losers_importance}

source("Functions/amce.R")

res_main <-  main_02 %>%
  amce(post_fair, treat_winningmargin, treat_winnergloat, treat_prime, treat_messenger, subgroup = 'importance') 
...
kable(res_main, booktabs = TRUE, caption = "Average Marginal Component Effect -- Norwegian conjoint experiment", col.names = linebreak(c("Treatment value", "Estimate", "Std. Error", "t-statistic", "p value", "Issue importance")), digits = 2) %>% 
  kable_styling(bootstrap_options = c("striped", "hover", "responsive")) %>% 
 kableExtra::group_rows(index = c("Winning margin" = 6, "Winner gloating" = 4, "Good loser prime" = 8, "Messenger" = 4))
