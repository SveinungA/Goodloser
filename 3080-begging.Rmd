...
```{r 309_begging_post_fair_losers}
  ...
  scale_x_continuous(
   ...
   labs(
    x = "Marginal effect, perceived fairness of decision",
       y = "",
    title = 'Average Marginal Component Effect -- Norwegian conjoint experiment',
    subtitle = 'Sample: Respondents who evaluated the ban on begging issue and experienced an unfavorable outcome') +
  theme_bw() +
  theme(plot.margin = unit(c(2, 2, 2, 2), "mm"),  plot.title = element_text(size=9), plot.subtitle = element_text(size=8), strip.text.y = element_text(size=5, angle=0) ,      axis.text.x=element_text(size=rel(0.7)))
fig_amce

ggsave(
  here("output", "nocon", "figs", "pngs", "fig_begging_fair-losers.png"),
  plot = fig_amce,
  width = 6, height = 2.75
)

ggsave(
  here("output", "nocon", "figs", "pdfs", "fig_begging_fair_losers.pdf"),
  plot = fig_amce,
  width = 6, height = 2.75
)
...
kable(res_main, booktabs = TRUE, caption = 'Average Marginal Component Effect -- Norwegian conjoint experiment', col.names = linebreak(c("Treatment value", "Estimate", "Std. Error", "t-statistic", "p value")), digits = 2) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "responsive")) %>% 
 group_rows(index = c("Winning margin" = 3, "Winner gloating" = 2, "Good loser prime" = 4, "Messenger" = 2))%>% 
  cat(., file = here("output", "nocon", "tbls", "html",  "tbl-bgging-fairness-losers-nocon.html"))
```